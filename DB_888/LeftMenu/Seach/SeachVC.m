...
- (void)loadupViews
{
    ...
    _seachView.frame = CGRectMake(0, 0, kDeviceWidth, _seachView.frame.size.height);
...
- (void)loadSeachViews
{
    s = [[UIScrollView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(_seachView.frame), kDeviceWidth, KDeviceHeight- CGRectGetMaxY(_seachView.frame))];
    ...
    //创建删除历史按钮
    UIView *footer = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kDeviceWidth, 100)];
    ...
    //获取搜索历史
    [self getHistory];
    _hotTab = [[SeachTextTab alloc]initWithFrame:CGRectMake(kDeviceWidth, 0, s.frame.size.width, s.frame.size.height)];
    _hotTab.father = self;
    s.contentSize = CGSizeMake(kDeviceWidth*2, 0);
    s.pagingEnabled = YES;
    s.delegate = self;
...
//    NSArray *his = [[WYDataBase sharedDataBase]getAllSeachText];
    ...
    if (ddata.count == 0) {
        ...
        s.contentOffset = CGPointMake(kDeviceWidth, 0);
    }
}
//请求热搜词
...
- (void)changeoffsetByIndex:(NSInteger)index
{
    [UIView animateWithDuration:0.20 animations:^{
        s.contentOffset = CGPointMake(kDeviceWidth*index, 0);
    }];
}
